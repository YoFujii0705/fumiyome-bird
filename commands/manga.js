...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('schedule')
        .setDescription('📅 更新通知設定一覧を表示'))
    
    // 🆕 通知設定用サブコマンド（scheduleのエイリアス）
    .addSubcommand(subcommand =>
      subcommand
        .setName('notifications')
        .setDescription('🔔 通知設定一覧を表示')),
  ...
  examples: [
    ...
    '/manga add title:ワンピース author:尾田栄一郎 type:series format:volume status:ongoing update_schedule:weekly-monday',
    ...
    '/manga info （選択式）',
    '// 🧪 開発・テスト用コマンド',
    '/manga test action:notification manga_id:1',
    '/manga test action:check_status',
    '/manga debug',
    '/manga schedule',
    '/manga notifications'
  ],
  ...
  help: {
    description: '漫画の管理機能です。読みたい漫画リストの管理と読書記録をサポートします。巻数・話数ごとの進捗管理と更新通知機能も含まれています。',
    usage: '以下のサブコマンドが利用できます：',
    subcommands: {
      add: '新しい漫画を読みたいリストに登録します。作品タイプと形式の指定が必要です。更新スケジュールも設定可能。',
      read: '特定の巻数/話数を読了として記録します。（選択式）',
      start: '漫画の読書を開始し、読書中ステータスに変更します。通知も有効化されます。（選択式）',
      ...
      test: '🧪 通知機能のテスト用コマンド（開発・デバッグ用）',
      debug: '🔧 通知設定のデバッグ情報を表示（開発用）',
      schedule: '📅 設定されている更新通知一覧を表示',
      notifications: '🔔 通知設定一覧を表示（scheduleのエイリアス）'
    },
    tips: [
